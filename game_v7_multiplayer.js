...
class AccountManager {
    constructor() {
        this.ACCOUNT_LIST_KEY = 'sanguo_v7_accounts';
        this.CURRENT_ACCOUNT_KEY = 'sanguo_v7_current';
    }
    ...
    getDefaultPlayerData() {
        return {
            level: 1, exp: 0, gold: 1000, gem: 100,
            heroes: [{
                id: 'zhaolei', name: '赵累', rarity: 'R',
                hp: 150, attack: 35, defense: 25, speed: 20
            }],
            team: ['zhaolei'],
            passedLevels: [],
            currentLevel: 1,
...
class Game {
    constructor() {
        ...
        this.state = { screen: 'login', battle: null };
    ...
    init() {
        console.log('🎮 三国英雄传 v7.0 多人版 初始化中...');
            ...
            <div id="shop-modal" class="modal">
                <div class="modal-content">
                    ...
                    <div class="shop-items">
                        <div class="shop-item" style="display: flex; justify-content: space-between; align-items: center; padding: 15px; background: #f5f5f5; border-radius: 10px;">
                            <span>📜 经验书</span>
                            <span>💰 100</span>
        ...
        slotsDiv.innerHTML = `
            <div style="display: flex; gap: 10px; justify-content: center; flex-wrap: wrap;">
                ${Array(5).fill(null).map((_, i) => {
                    ...
                    return `<div style="width: 60px; height: 60px; background: ${hero ? '#3498db' : '#ecf0f1'}; border-radius: 10px; display: flex; align-items: center; justify-content: center; font-size: 12px;">${hero ? hero.name.substring(0, 2) : '空'}</div>`;
                }).join('')}
            </div>
        `;
    }
                ...
                <div style="background: ${isUnlocked ? 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' : '#555'}; padding: 20px; border-radius: 15px; margin-bottom: 15px; opacity: ${isUnlocked ? 1 : 0.6};">
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        <div style="flex: 1;">
                            ...
                            <div style="font-size: 14px;">🎁 奖励：💰${level.reward.gold} ✨${level.reward.exp} ${level.reward.gem > 0 ? '💎' + level.reward.gem : ''}</div>
                        </div>
                        ${isUnlocked ? `
        ...
        content.innerHTML = `
            <div style="padding: 20px;">
                ...
                <div style="margin-top: 30px; text-align: center;">
                    <button class="btn btn-danger" onclick="game.endBattle(true)">🏃 逃跑</button>
                </div>
            </div>
        `;
    }
